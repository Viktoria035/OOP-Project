...
void System::cancel_order(size_t orderID)
{
	if (currentUser->getType() == "client")
	{
		...
		if (arrOrders[idx].getStatus() == StatusOrder::awaitingDriver || arrOrders[idx].getStatus() == StatusOrder::accepted || arrOrders[idx].getStatus() == StatusOrder::inProgress)
		{
...
size_t System::findClosestDriver(const Address &adr)
{
	...
	if (i >= size)
		throw std::logic_error("Unexpected code path in findClosestDriver.");
	return i;
}
