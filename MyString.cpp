...
MyString::MyString(const char* str, const char* begin, const char* end,const size_t MAX) :MyString(begin > end ? (MAX - (begin - end) + 1) : ((end - begin) + 1))
{
	int i = 0;
	int startIndex = begin - str;
	int endIndex = end - str;
	int diff = startIndex > endIndex ? MAX - (startIndex - endIndex) : endIndex - startIndex;
	while (i != diff)
	{
		if (begin == &str[MAX - 1])
			begin = str;
...
void MyString::copyFrom(const MyString& other)
{
	...
	strcpy(_data, other._data);// now is here <-----
}
