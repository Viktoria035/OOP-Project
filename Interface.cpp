...
namespace
{
	bool isClientCommandsCorrect(const MyString& command)
	{
		return command == "create_order" || command == "check_order" || command == "cancel_order"
			|| command == "pay_driver" || command == "rate_driver" || command == "add_money" || command == "check_ordersid"
			|| command == "notifications" || command == "logout";
	}
	...
	bool isDriverCommandsCorrect(const MyString& command)
	{
		return command == "change_address" || command == "check_messages" || command == "accept_order"
			|| command == "decline_order" || command == "finish_order" || command == "accept_payment"
			|| command == "notifications" || command == "logout";
	}
...
void Interface::run()
{
	...
	std::cout << ">>*------------------------------------------------------*<<" << std::endl;
	std::cout << "You have successfully logged out! Have a nice day!" << std::endl;
}